...
class SizeCompareBranches(BuildScriptBase):
    ...
    def compare_results_object_files(self, result_master: Result, result_branch: Result):
        ...
        for vehicle in result_master.vehicle.keys():
            if "board_dir" in result_master.vehicle[vehicle]:
                board_dir_master = result_master.vehicle[vehicle]["board_dir"]
                break
        for vehicle in result_branch.vehicle.keys():
            if "board_dir" in result_branch.vehicle[vehicle]:
                board_dir_branch = result_branch.vehicle[vehicle]["board_dir"]
                break
