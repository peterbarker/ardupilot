...
switch value'''
    ...
    def assert_ekfs_match_sim_state(self,
        ...
        for key in ekf_message_types:
            ...
            te = rows[:, 0]
            armed = np.zeros(len(te), dtype=bool)
            for (t0, t1) in armed_spans:
                armed |= (te >= t0) & (te <= t1)
            # restrict to samples bracketed by truth so interpolation
            # never extrapolates:
            armed &= (te >= max(sim[0, 0], sim2[0, 0])) & (te <= min(sim[-1, 0], sim2[-1, 0]))
            rows = rows[armed]
            te = rows[:, 0]
            if len(te) < min_samples:
                raise NotAchievedException(
                    "Insufficient %s/truth samples compared (%u)" % (key, len(te)))

            roll_err = np.abs(rows[:, 1] - np.interp(te, sim[:, 0], sim[:, 1]))
            pitch_err = np.abs(rows[:, 2] - np.interp(te, sim[:, 0], sim[:, 2]))
            yaw_err = np.abs((rows[:, 3] - np.interp(te, sim[:, 0], sim_yaw_unwrapped) + 180) % 360 - 180)
            vel_err = np.sqrt(
                (rows[:, 4] - np.interp(te, sim2[:, 0], sim2[:, 1]))**2 +
                (rows[:, 5] - np.interp(te, sim2[:, 0], sim2[:, 2]))**2 +
                (rows[:, 6] - np.interp(te, sim2[:, 0], sim2[:, 3]))**2)
            ...
            pn_err = rows[:, 7] - np.interp(te, sim2[:, 0], sim2[:, 4])
            pe_err = rows[:, 8] - np.interp(te, sim2[:, 0], sim2[:, 5])
            pd_err = rows[:, 9] - np.interp(te, sim2[:, 0], sim2[:, 6])
            nbase = min(10, len(te))
            ...
            considered = te >= ignore_before_time_s
            bad &= considered
            ...
            for i in range(len(te)):
                ...
                if run_start is None:
                    run_start = te[i]
                    self.progress("%s transient %s divergence at t=%.3f" % (key, " ".join(desc), te[i]))
                elif te[i] - run_start > max_violation_duration_s:
                    raise NotAchievedException(
                        "%s diverged from truth for more than %.1fs (t=%.3f %s)" %
                        (key, max_violation_duration_s, te[i], " ".join(desc)))
