...
void QuadPlane::setup_target_position(void)
{
    ...
    poscontrol.target_neu_m.z = plane.next_WP_loc_abs_alt_m() - origin.get_alt_m();
    ...
    */
    ...
    if (plane.control_mode == &plane.mode_guided && guided_takeoff) {
        ...
        if (ahrs.get_origin(origin)) {
            ...
            float pos_u_m = margin_m + plane.next_WP_loc_abs_alt_m() - origin.get_alt_m()
            vel_u_ms = 0;
...
bool QuadPlane::do_vtol_takeoff(const AP_Mission::Mission_Command& cmd)
{
    ...
    const float vel_max = MAX(0.1, pilot_speed_z_max_up_ms);
    const float vel_z = inertial_nav.get_velocity_z_up_cms() * 0.01f;
    const float t_accel = (vel_max_ms - vel_u_ms) / accel_m_s_s;
    const float d_accel = vel_u_ms * t_accel_s + 0.5f * accel_m_s_s * sq(t_accel_s);
...
void QuadPlane::guided_start(void)
{
    ...
    if (plane.current_loc.get_alt_cm(Location::AltFrame::ABSOLUTE, from_alt) && plane.next_WP_loc.get_alt_cm(Location::AltFrame::ABSOLUTE, to_alt)) {
        ...
        poscontrol.slow_descent = (plane.current_loc.get_alt_cm() > plane.next_WP_loc_abs_alt_m());
    }
}
