...
bool Copter::being_tuned(RCTuningFunction tuning_param) const
{
...
void Copter::tuning(const RC_Channel *tuning_ch, RCTuningFunction tuning_param, float tuning_min, float tuning_max)
{
    ...
    case RCTuningFunction::CIRCLE_RATE:
        circle_nav->set_rate_degs(tuning_value);
        break;
#endif

    case RCTuningFunction::RC_FEEL_RP:
        attitude_control->set_input_tc(tuning_value);
        break;

    case RCTuningFunction::RATE_PITCH_KP:
        attitude_control->get_rate_pitch_pid().set_kP(tuning_value);
        break;

    case RCTuningFunction::RATE_PITCH_KI:
        attitude_control->get_rate_pitch_pid().set_kI(tuning_value);
        break;

    case RCTuningFunction::RATE_PITCH_KD:
        attitude_control->get_rate_pitch_pid().set_kD(tuning_value);
        break;

    case RCTuningFunction::RATE_ROLL_KP:
        attitude_control->get_rate_roll_pid().set_kP(tuning_value);
        break;

    case RCTuningFunction::RATE_ROLL_KI:
        attitude_control->get_rate_roll_pid().set_kI(tuning_value);
        break;

    case RCTuningFunction::RATE_ROLL_KD:
        attitude_control->get_rate_roll_pid().set_kD(tuning_value);
        break;

#if FRAME_CONFIG != HELI_FRAME
    case RCTuningFunction::RATE_MOT_YAW_HEADROOM:
        motors->set_yaw_headroom(tuning_value);
        break;
#endif

    case RCTuningFunction::RATE_YAW_FILT:
        attitude_control->get_rate_yaw_pid().set_filt_E_hz(tuning_value);
        break;

    case RCTuningFunction::SYSTEM_ID_MAGNITUDE:
#if MODE_SYSTEMID_ENABLED
        copter.mode_systemid.set_magnitude(tuning_value);
#endif
        break;

    case RCTuningFunction::POS_CONTROL_ANGLE_MAX:
        pos_control->set_lean_angle_max_deg(tuning_value);
        break;

    case RCTuningFunction::LOITER_MAX_XY_SPEED:
        loiter_nav->set_speed_max_NE_ms(tuning_value);
        break;

    case RCTuningFunction::WP_SPEED_MS:
