...
void ModeRTL::descent_run()
{
    ...
    // WP_Nav has set the vertical position control targets
    // run the vertical position controller and set output throttle
    int32_t above_home_alt_cm;
    UNUSED_RESULT(rtl_path.descent_target.get_alt_cm(Location::AltFrame::ABOVE_HOME, above_home_alt_cm));
    pos_control->set_alt_target_with_slew_cm(above_home_alt_cm);
    pos_control->update_U_controller();
    ...
    _state_complete = labs(rtl_path.descent_target.get_alt_cm() - pos_control->get_pos_estimate_NEU_cm().z) < 20;
}
...
void ModeRTL::compute_return_target()
{
    ...
    int32_t curr_alt = copter.current_alt_above_home_cm() - pos_offset_z;
    ...
    UNUSED_RESULT(rtl_path.return_target.get_alt_cm(rtl_path.return_target.get_alt_frame(), target_alt));
    target_alt = MAX(target_alt, 0);
    ...
    if ((copter.fence.get_enabled_fences() & AC_FENCE_TYPE_ALT_MAX) != 0) {
        ...
        if (rtl_path.return_target.get_alt_cm(Location::AltFrame::ABOVE_HOME, target_alt)) {
            ...
            if (target_alt > fence_alt) {
                ...
                rtl_path.return_target.offset_up_cm(-(target_alt - fence_alt));
            }
        }
    }
#endif
    ...
    int32_t return_target_alt_cm;
    UNUSED_RESULT(rtl_path.return_target.get_alt_cm(rtl_path.return_target.get_alt_frame(), return_target_alt_cm));
    rtl_path.return_target.set_alt_cm(MAX(return_target_alt_cm, curr_alt), rtl_path.return_target.get_alt_frame());
}
