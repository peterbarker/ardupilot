...
void Copter::parachute_check()
{
    ...
    if (control_loss_count == 0 && parachute.alt_min() != 0 && (current_alt_above_home_cm() < (int32_t)parachute.alt_min() * 100)) {
        return;
    }
...
void Copter::parachute_manual_release()
{
    ...
    // do not release if we are landed or below the minimum altitude above home
    if ((parachute.alt_min() != 0 && (current_alt_above_home_cm() < parachute.alt_min()*100))) {
        // warn user of reason for failure
