...
void Copter::thrust_loss_check()
{
    ...
    if (copter.option_enabled(FlightOption::DISABLE_THRUST_LOSS_CHECK)) {
        return;
    }
    ...
    if (thrust_loss_counter >= (THRUST_LOSS_CHECK_TRIGGER_SEC * scheduler.get_loop_rate_hz())) {
        ...
        if (copter.option_enabled(FlightOption::RELEASE_GRIPPER_ON_THRUST_LOSS)) {
            gripper.release();
        }
#endif
    }
...
{
    // no-op if suppressed by flight options param
    if (copter.option_enabled(FlightOption::DISABLE_YAW_IMBALANCE_WARNING)) {
        return;
    }
