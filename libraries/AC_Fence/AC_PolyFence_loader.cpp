...
bool AC_PolyFence_loader::scan_eeprom(void (*callback)(AC_PolyFenceType type, uint16_t offset))
{
    ...
    if ((magic & 0xff) != new_fence_storage_magic) {
        Debug("fence magic mismatch (%u) (%u)", (unsigned)magic, (unsigned)new_fence_storage_magic);
        return false;
    }
    read_offset+=4;
    bool all_done = false;
...
bool AC_PolyFence_loader::index_eeprom()
{
    ...
    if (_eeprom_fence_count) {
        ...
        Debug("Fence: Allocating %u bytes for index", (unsigned)index_allocation_size);
...
bool AC_PolyFence_loader::load_from_eeprom()
{
    ...
    const uint32_t magic = fence_storage.read_uint32(read_offset);
    if ((magic & 0xff) != new_fence_storage_magic) {
        gcs().send_text(MAV_SEVERITY_WARNING, "fence magic mismatch (%u) (%u)", (unsigned)magic, (unsigned)new_fence_storage_magic);
        return false;
    }
    ...
    { // allocate general boundary point array:
        ...
        Debug("Fence: Allocating %u bytes for points", (unsigned)allocation_size);
    ...
    { // allocate storage for inclusion polyfences:
        ...
        Debug("Fence: Allocating %u bytes for inc. fences", (unsigned)allocation_size);
    ...
    { // allocate storage for exclusion polyfences:
        ...
        Debug("Fence: Allocating %u bytes for exc. fences", (unsigned)allocation_size);
    ...
    { // allocate storage for circular inclusion fences:
        ...
        Debug("Fence: Allocating %u bytes for circ. inc. fences", (unsigned)allocation_size);
    ...
    { // allocate storage for circular exclusion fences:
        ...
        Debug("Fence: Allocating %u bytes for circ. exc. fences", (unsigned)allocation_size);
