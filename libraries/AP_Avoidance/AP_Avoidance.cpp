...
float closest_approach_z(const AbsAltLocation &my_loc,
                         const Vector3f &my_vel,
                         ...
                         const uint8_t time_horizon)
{
    ...
    float delta_pos_d = obstacle_loc.get_alt_cm() - my_loc.get_alt_cm();
...
void AP_Avoidance::update_threat_level(const AbsAltLocation &my_loc,
                                       const Vector3f &my_vel,
    ...
    if (threat != nullptr) {
        ...
        if (new_threat_level == MAV_COLLISION_THREAT_LEVEL_HIGH) {
            ...
            if (action != MAV_COLLISION_ACTION_NONE && _fail_altitude_minimum > 0 &&
                AP::ahrs().get_location(my_loc) && (my_loc.get_alt_m() < _fail_altitude_minimum)) {
...
bool AP_Avoidance::get_vector_perpendicular(const AP_Avoidance::Obstacle *obstacle, Vector3f &vec_neu) const
{
    ...
    if (obstacle->_velocity.length() < _low_velocity_threshold) {
        ...
        const float delta_pos_z = my_abs_pos.get_alt_cm() - obstacle->_location.get_alt_cm();
...
// v1 is NED
Vector3f AP_Avoidance::perpendicular_xyz(const AbsAltLocation &p1, const Vector3f &v1, const AbsAltLocation &p2)
{
    const Vector2f delta_p_2d = p1.get_distance_NE(p2);
    Vector3f delta_p_xyz = Vector3f(delta_p_2d[0],delta_p_2d[1],(p2.get_alt_m()-p1.get_alt_m())); //check this line
...
// v1 is NED
Vector2f AP_Avoidance::perpendicular_xy(const AbsAltLocation &p1, const Vector3f &v1, const AbsAltLocation &p2)
{
