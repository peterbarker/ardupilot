...
{
    _dev = hal.i2c_mgr->get_device(i2c_bus, i2c_address, 100000, false, 20);
    if (!_dev) {
        return;
    }else {
        ...
        if(write_word(REG_ACCESS_CODE, CUSTOMER_CODE)){
            ::printf("ACS37800-> Wrote access code successful\n");
        }
        hal.scheduler->delay(100);
        write_word(REG_F_SHADOW, CONFIG_F);
        uint32_t c_config;
        read_word(REG_C_SHADOW, c_config);
        ::printf("ACS37800-> REG_SHADOW_C(read): %x\n", c_config);
        c_config |= CONFIG_C_MASK;
        write_word(REG_C_SHADOW, c_config);
        ::printf("ACS37800-> REG_SHADOW_C(wrote): %x\n", c_config);
        hal.scheduler->delay(100);
        // write_word(REG_F_EEPROM, CONFIG_F);
        // hal.scheduler->delay(100);
        uint32_t check_c_config;
        read_word(REG_C_SHADOW,check_c_config);
        ::printf("ACS37800-> REG_SHADOW_C(read after write): %x\n",check_c_config);
...
{
    return (((float) icode)/32768.0)*90.0*curr_fine_mult; //more res because of bypass_N_ena?
}
