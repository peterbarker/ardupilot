...
void Viewpro::send_t1_f1_b1_d1()
{
    ...
    // yaw: int16, 1 bit = 360/65536 degrees
    const int16_t yaw_out   = (int16_t)(degrees(joint_angles.z) * VIEWPRO_DEG_TO_OUTPUT);
    // pitch: driver negates on parse, so we send negative of our pitch
    const int16_t pitch_out = (int16_t)(-degrees(joint_angles.y) * VIEWPRO_DEG_TO_OUTPUT);
    // roll: 12-bit, 0..4095 = -90..+90 deg
    const float roll_deg = constrain_float(degrees(joint_angles.x), -90.0f, 90.0f);
    const uint16_t roll_out = (uint16_t)((roll_deg + 90.0f) * 4095.0f / 180.0f);
