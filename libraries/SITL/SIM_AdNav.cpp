        ...
        while((an_packet = an_packet_decode(&_an_decoder)) != NULL)
        {
            switch(an_packet->id){
                ...
                case AN_PACKET_ID_REQUEST_PACKET:
                    ...
                    break;
                default:
                    printf("AN_DEVICE_SIM: Unknown AN Packet");
                    break;
            }
        }
    }
...
void AdNav::send_state_pkt(){
    ...
    system_state_packet.latitude = radians(fdm.latitude);
    system_state_packet.longitude = radians(fdm.longitude);
    ...
    system_state_packet.orientation[2] = radians(fdm.heading);
