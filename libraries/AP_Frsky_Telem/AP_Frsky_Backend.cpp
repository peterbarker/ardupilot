...
void AP_Frsky_Backend::calc_gps_position(void)
{
    ...
    if (_ahrs.get_location(loc)) {
        ...
        _SPort_data.latdddmm = uint16_t(lat);
        _SPort_data.latmmmm = uint16_t((lat - _SPort_data.latdddmm) * 10000);
        _SPort_data.lat_ns = (loc.lat < 0) ? 'S' : 'N';

        float lon = format_gps(fabsf(loc.lng/10000000.0f));
        _SPort_data.londddmm = uint16_t(lon);
        _SPort_data.lonmmmm = uint16_t((lon - _SPort_data.londddmm) * 10000);
