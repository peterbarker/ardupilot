...
// @Field: IDX: Index of the publishing ECU
    AP::logger().WriteStreaming("EFI",
                       ...
                       double(state.spark_dwell_time_ms),
                       double(state.atmospheric_pressure_kpa),
                       double(state.intake_manifold_pressure_kpa),
                       double(KELVIN_TO_C(state.intake_manifold_temperature)),
                       double(KELVIN_TO_C(state.coolant_temperature)),
                       double(state.oil_pressure),
                       double(KELVIN_TO_C(state.oil_temperature)),
                       double(state.fuel_pressure),
                       double(state.fuel_consumption_rate_cm3pm),
                       double(state.estimated_consumed_fuel_volume_cm3),
...
// @Field: IDX: Index of the publishing ECU
    AP::logger().WriteStreaming("ECYL",
                                ...
                                AP_HAL::micros64(),
                                0,
                                double(state.cylinder_status.ignition_timing_deg),
                                double(state.cylinder_status.injection_time_ms),
                                double(KELVIN_TO_C(state.cylinder_status.cylinder_head_temperature)),
                                double(KELVIN_TO_C(state.cylinder_status.exhaust_gas_temperature)),
                                double(state.cylinder_status.lambda_coefficient),
                                double(KELVIN_TO_C(state.cylinder_status.cylinder_head_temperature2)),
                                double(KELVIN_TO_C(state.cylinder_status.exhaust_gas_temperature2)),
                                state.ecu_index);
}
