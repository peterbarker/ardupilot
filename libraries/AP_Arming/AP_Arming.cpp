    ...
    if (check_enabled(Check::AIRSPEED)) {
        ...
        for (uint8_t i=0; i<AIRSPEED_MAX_SENSORS; i++) {
            if (airspeed->enabled(i) && airspeed->use(i) && !airspeed->healthy(i)) {
                check_failed(Check::AIRSPEED, report, "Airspeed %d not healthy", i + 1);
                return false;
            }
        }
    }
...
bool AP_Arming::gps_checks(bool report)
{
    ...
    if (check_enabled(Check::GPS)) {
        ...
        if (!gps.blend_health_check()) {
            check_failed(Check::GPS, report, "GPS blending unhealthy");
            return false;
        }
#endif
...
bool AP_Arming::rc_arm_checks(AP_Arming::Method method)
{
    ...
    if (rcmap != nullptr) {
            ...
            for (uint8_t i = 0; i < ARRAY_SIZE(channels); i++) {
                ...
                if (c->get_control_in() != 0) {
                    if ((method != Method::RUDDER) || (c != rc().get_arming_channel())) { // ignore the yaw input channel if rudder arming
                        check_failed(Check::RC, true, "%s (RC%d) is not neutral", names[i], channels[i]);
                        check_passed = false;
                    }
                }
            }
        ...
        if (rc().arming_check_throttle()) {
            ...
            if (c != nullptr) {
                if (c->get_control_in() != 0) {
                    check_failed(Check::RC, true, "Throttle (RC%d) is not neutral", rcmap->throttle());
                    check_passed = false;
                }
            }
