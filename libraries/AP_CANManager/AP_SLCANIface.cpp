...
bool SLCAN::CANIface::handle_FrameDataExt(const char* cmd, bool canfd)
{
    ...
    f.id = f.FlagEFF | id;
    if (!hex_to_uint8(cmd[9], f.dlc) || f.dlc > (canfd?15:8)) {
        return false;
    }
...
bool SLCAN::CANIface::handle_FDFrameDataExt(const char* cmd)
{
    ...
    f.id = f.FlagEFF | id;
    if (!hex_to_uint8(cmd[9], f.dlc) || f.dlc > AP_HAL::CANFrame::dataLengthToDlc(AP_HAL::CANFrame::MaxDataLen)) {
        return false;
    }
...
bool SLCAN::CANIface::handle_FrameDataStd(const char* cmd)
{
    AP_HAL::CANFrame f {};
    uint32_t id;
    if (!hex_chars_to_uint32(&cmd[1], 3, id)) {
        return false;
    }
    f.id = id;
