...
void AP_MotorsSingle::output_to_motors()
{
    ...
    switch (_spool_state) {
        case SpoolState::SHUT_DOWN:
            ...
            rc_write_angle(AP_MOTORS_MOT_1, int16_t(_roll_radio_passthrough * AP_MOTORS_SINGLE_SERVO_INPUT_RANGE));
            rc_write_angle(AP_MOTORS_MOT_2, int16_t(_pitch_radio_passthrough * AP_MOTORS_SINGLE_SERVO_INPUT_RANGE));
            rc_write_angle(AP_MOTORS_MOT_3, int16_t(-_roll_radio_passthrough * AP_MOTORS_SINGLE_SERVO_INPUT_RANGE));
            rc_write_angle(AP_MOTORS_MOT_4, int16_t(-_pitch_radio_passthrough * AP_MOTORS_SINGLE_SERVO_INPUT_RANGE));
        ...
        case SpoolState::GROUND_IDLE:
            ...
            for (uint8_t i = 0; i < NUM_ACTUATORS; i++) {
                rc_write_angle(AP_MOTORS_MOT_1 + i, int16_t(_spin_up_ratio * _actuator_out[i] * AP_MOTORS_SINGLE_SERVO_INPUT_RANGE));
            }
        ...
        case SpoolState::SPOOLING_DOWN:
            ...
            for (uint8_t i = 0; i < NUM_ACTUATORS; i++) {
                rc_write_angle(AP_MOTORS_MOT_1 + i, int16_t(_actuator_out[i] * AP_MOTORS_SINGLE_SERVO_INPUT_RANGE));
            }
