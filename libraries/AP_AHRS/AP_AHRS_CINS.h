...
class AP_AHRS_CINS : public AP_AHRS_Backend
{
public:
    AP_AHRS_CINS();
    void init(void) override;

    const char *shortname() const override { return "CINS"; }
    ...
    void reset_gyro_drift(void) override { }  // FIXME

    // reset the current attitude, used on new IMU calibration
    void reset() override { }  // FIXME

    // return a wind estimation vector, in m/s
    bool wind_estimate(Vector3f &wind) const override { return false; }
    ...
    void get_control_limits(float &ekfGndSpdLimit, float &ekfNavVelGainScaler) const override {
        // lower gains in VTOL controllers when flying on CINS
        ekfGndSpdLimit = 50.0;
