...
gcs:send_text(6, string.format("CFG: v3.1 FSO starting"))
...
if string.find(FWVersion:string(), "ArduCopter V4.5%.7%-C2%.4") == nil then
   gcs:send_text(3, string.format("CFG: ERROR: unsupported firmware '%s', expected ArduCopter V4.5.7-C2.4", FWVersion:string()))
   ...
   PAY = {
      ...
      profiles = {
         ...
         [6] = {
            name = "Riegl Lidar",
            params = {
               ["SERVO9_REVERSED"] = 1,         -- Reverse output
               ["SERVO9_FUNCTION"] = 147,       -- Switch 4 scaled mapping
               ["SERVO9_MAX"] = 2000,           -- Lidar max PWM (from lidar script)
            }
         },
         [10] = {
            name = "Silvus",
         ...
         [12] = {
            name = "Silvus/CM62 - HookFixed", --Ap perif can node + flush hook as gripper
            params = {
               ["CAN_D1_UC_SER_EN"] = 1,
         ...
         [30] = {
            ...
            params = {
               ...
               ["RC6_OPTION"] = 167,      -- camera zoom
               ["RC9_OPTION"] = 163,      -- yaw lock/follow toggle
...
local function handle_show_domain_statuses()
   ...
   for _, domain in pairs(config_domains) do
      ...
      if sel_value == SEL_APPLY_DEFAULTS then
         send_text(6, string.format("%s: using defaults", domain.param_name))
         goto hsds_next_domain
      end
      ...
      send_text(6, string.format("%s: using profile %s", domain.param_name, profile.name))
      ::hsds_next_domain::
   end
end
