...
{
    const char* vname = get_camera_vendor_name();
    if (vname == nullptr) {
        return;
    }

    uint8_t vendor_name[MAVLINK_MSG_CAMERA_INFORMATION_FIELD_VENDOR_NAME_LEN] {};
    strncpy_noterm((char*)vendor_name, vname, sizeof(vendor_name));

    uint8_t model_name[MAVLINK_MSG_CAMERA_INFORMATION_FIELD_MODEL_NAME_LEN] {};
    strncpy_noterm((char*)model_name, get_camera_model_name(), sizeof(model_name));
