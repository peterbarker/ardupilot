...
void AP_GPS_MAV::handle_msg(const mavlink_message_t &msg)
{
    switch (msg.msgid) {
        ...
        case MAVLINK_MSG_ID_GPS_INPUT: {
            ...
            state.status = (AP_GPS_FixType)packet.fix_type;

            Location loc = {};
            loc.lat = packet.lat;
            loc.lng = packet.lon;
            if (have_alt) {
                loc.alt = int32_t(packet.alt * 100); // convert to centimeters
            }
            state.location = loc;

            if (have_hdop) {
                state.hdop = uint16_t(packet.hdop * 100); // convert to centimeters
            }

            if (have_vdop) {
                state.vdop = uint16_t(packet.vdop * 100); // convert to centimeters
            }
