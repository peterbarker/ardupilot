...
const AP_Param::GroupInfo ModeDock::var_info[] = {
    ...
    // @Description: Direction in which vehicle should approach towards dock. 
    // @Units: deg
    // @Range: 0 360
    // @Increment: 1
    // @User: Advanced
    ...
    // @User: Advanced
    AP_GROUPINFO("_HDG_CORR_EN", 3, ModeDock, hdg_corr_enable, 1),

    // @Param: _HDG_CORR_WT
    // @DisplayName: Dock mode heading correction weight
    // @Description: This value describes how aggressively vehicle tries to correct it's heading to be on desired line of approach
    // @Range: 0.00 0.90
    // @Increment: 0.05
    // @User: Advanced
    ...
    // @Description: The vehicle starts stopping when it is this distance way from docking target
    // @Units: m
    // @Range: 0 2
    // @Increment: 0.01
    // @User: Standard
...
bool ModeDock::_enter()
{
    ...
    if (hdg_corr_enable && is_negative(desired_dir)) {
        ...
        gcs().send_text(MAV_SEVERITY_ERROR, "Dock: Set DOCK_DIR or disable heading correction");
        return false;
    }
    ...
    // check if we need to dock in reverse direction
    _reversed = rover.precland.get_orient() == 4;
    g2.pos_control.set_reversed(_reversed);

    _loiter_after_docking = rover.is_boat();
...
void ModeDock::update()
{
    ...
    if (_docking_complete) {
        ...
        if (_loiter_after_docking) {
            // mode loiter must be initialised before calling update_post_docking method
    ...
    bool dock_in_sight = rover.precland.get_target_position_cm(_dock_pos_rel_origin_cm);
    ...
    // construct unit vector in the desired direction from where we want the vehicle approach the dock
    // this helps to dock the vehicle head-on even when we enter the dock mode at an angle towards the dock
    const Vector2f desired_heading_NE = Vector2f{cosf(radians(desired_dir)), sinf(radians(desired_dir))};
    ...
    bool force_real_target = _distance_to_destination < _force_real_target_limit_cm * 0.01f;
    ...
    if (_distance_to_destination <= stopping_dist || (force_real_target && !dock_in_sight)) {
        _docking_complete = true;

        // send a one time notification to GCS
        gcs().send_text(MAV_SEVERITY_INFO, "Dock: Docking complete");

        // initialise mode loiter if we need to loiter after docking completion
        if (_loiter_after_docking) {
            // if we fail to enter, we set _loiter_after_docking to false
            _loiter_after_docking = rover.mode_loiter.enter();}
        return;
    }
    ...
    // as the vehicle tries to reach this target, this target appears to move towards the dock and at last it is sandwhiched b/w dock and vehicle
    ...
    if  (!force_real_target && hdg_corr_enable) {
        float correction_vec_mag = hdg_corr_weight * dock_pos_rel_vehicle_cm.projected(desired_heading_NE).length();
        target_cm = _dock_pos_rel_origin_cm - desired_heading_NE * correction_vec_mag;
    }

    Vector2p target_pos = target_cm.topostype() * 0.01f;
...
float ModeDock::apply_slowdown(float desired_speed) {
    ...
    Matrix3f body_to_ned = AP::ahrs().get_rotation_body_to_ned();
    ...
    float error_ratio = target_error_cm / _acceptable_pos_error_cm_default;
...
bool ModeDock::calc_dock_pos_rel_vehicle_NE(Vector2f &dock_pos_rel_vehicle) {
